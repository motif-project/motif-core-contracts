...
contract RoleManager is Initializable, AccessControlEnumerableUpgradeable {
     ...
     */
    function setAdminRole(address _newAdmin) external onlyRole(ADMIN_ROLE) {
     ...
     */
   /* function setCuratorRole(address _curator) external {
     ...
     */
    function isAdminRoleMember(address _account) external view returns (bool) {
     ...
     */
    function isOwnerRoleMember(address _account) external view returns (bool) {
     ...
     */
    function isOperatorRoleMember(address _account) external view returns (bool) {
     ...
     */
    function isCuratorRoleMember(address _account) external view returns (bool) {
     ...
     */
    function getAdminRoleMemberAddress() external view returns (address) {
        uint256 adminCount = getRoleMemberCount(ADMIN_ROLE);
        if (adminCount == 0) {
            return address(0);
        }
     ...
     */
    function getCuratorRoleMemberAddress() external view returns (address) {
        uint256 curatorCount = getRoleMemberCount(CURATOR_ROLE);
        if (curatorCount == 0) {
            return address(0);
        }
     ...
     */
    function getOperatorRoleMemberAddress() external view virtual returns (address) {
